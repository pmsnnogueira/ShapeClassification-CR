function [case_library] = retain(case_library, new_case, journey_index)
    ...
    if ~isfield(new_case,'region')
        new_case.holiday_type = case_library(journey_index,5);
    end
    if ~isfield(new_case,'transportation')
        new_case.holiday_type = case_library(journey_index,6);
    end
    if ~isfield(new_case,'duration')
        new_case.holiday_type = case_library(journey_index,7);
    end
    if ~isfield(new_case,'seasion')
        new_case.holiday_type = case_library(journey_index,8);
    end
    if ~isfield(new_case,'accommodation')
        new_case.holiday_type = case_library(journey_index,9);
    end
